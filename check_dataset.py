...
if not os.path.exists(DATASET_PATH):
    ...
    for genre_folder in sorted(os.listdir(DATASET_PATH)):
        ...
        if os.path.isdir(genre_path):
            ...
            # Count only files so hidden entries or subfolders do not affect the result.
            number_of_files = sum(
                1 for item in os.listdir(genre_path)
                if os.path.isfile(os.path.join(genre_path, item))
            )
